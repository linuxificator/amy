...
| `hR`   | `reverb_room, reverb_room_level, reverb_room_liveness, reverb_room_damping, reverb_room_xover_hz` | `reverb_room` | int,float[,float,float,float] | Configure a built-in shared reverb: return index, level, liveness, damping and crossover. Shared returns must first be enabled with `max_reverb_rooms`. |
| `hS`   | `reverb_send_room, reverb_send_level` | `reverb_send` | int,float | Route the selected bus to a shared aux return with a weighted post-fader send. A send of zero excludes the bus while retaining its return selection. |
