...
Note 3: `SYNTH_FLAGS_IGNORE_NOTE_OFFS` is useful for one-shot sounds whose
natural decay should not be truncated. Such synths do not need forgotten-note
bookkeeping when voices are stolen; the rationale and regression coverage are
described in [Ignored note-offs and voice-stealing
bookkeeping](ignored-noteoff-bookkeeping.md).

Note 4: an `osc` number in a command addressed to a synth is **relative to the voice**, not absolute -- `osc=1` means the second osc of each of that synth's voices, wherever those oscs happen to live. The same goes for a parameter that names another osc: `mod_source`, `chained_osc` and `algo_source` are voice-relative too, so `amy.send(synth=1, osc=0, mod_source=1)` means "modulate osc 0 from osc 1 *of the same voice*".
