...
func _ready() -> void:
	_build_ui()
	_status.text = "Loading AMY Android backend..."
	...
	for _attempt in range(240):
		...
		var backend_status: String = str(_amy.call("status_text"))
		if not backend_status.is_empty() and backend_status != previous_status:
			previous_status = backend_status
			_status.text = backend_status
		await get_tree().create_timer(0.05).timeout

	_fail("AMY startup timeout: %s (rc=%s)" % [
		str(_amy.call("status_text")), str(_amy.call("last_error"))])
