...
// One slot per concurrently processed bus subset. Each core owns one slot;
// amy_fill_buffer reads them only after the platform join.
static volatile uint32_t fill_bus_fx_current_us[AMY_MAX_CORES];
static volatile uint32_t fill_bus_send_current_us[AMY_MAX_CORES];
...
// Process the existing AMY effect chain for one bus, then add that bus to its
// configured aux subset.  Buses are independent up to this point; a platform
// may therefore divide complete subsets over render cores without changing
// the DSP or routing model.
static AMY_IRAM_ATTR void amy_process_bus_builtin(uint16_t bus,
                                                  uint8_t diagnostic_slot) {
#ifdef AMY_ESP_LOAD_DIAGNOSTIC
    uint64_t part_started_us = amy_get_us();
#else
...
#ifdef AMY_ESP_LOAD_DIAGNOSTIC
    fill_bus_fx_current_us[diagnostic_slot] +=
        (uint32_t)(amy_get_us() - part_started_us);
    part_started_us = amy_get_us();
#endif
...
#ifdef AMY_ESP_LOAD_DIAGNOSTIC
    fill_bus_send_current_us[diagnostic_slot] +=
        (uint32_t)(amy_get_us() - part_started_us);
#endif
...
#ifdef AMY_ESP_LOAD_DIAGNOSTIC
    fill_bus_fx_current_us[partition] = 0;
    fill_bus_send_current_us[partition] = 0;
#endif
    for (uint16_t bus = 0; bus <= amy_global.highest_bus; ++bus) {
        if (amy_bus_partition(bus, partitions) == partition)
            amy_process_bus_builtin(bus, partition);
    }
}
...
int16_t * amy_fill_buffer() {
    ...
    for (uint8_t slot = 0; slot < AMY_MAX_CORES; ++slot) {
        fill_bus_fx_current_us[slot] = 0;
        fill_bus_send_current_us[slot] = 0;
    }
#endif
    ...
    if (!serial_bus_hooks) {
        ...
        for (int bus=0; bus <= amy_global.highest_bus; ++bus) {
            amy_process_bus_builtin(bus, 0);
#ifdef AMY_ESP_LOAD_DIAGNOSTIC
            uint64_t hook_started_us = amy_get_us();
#endif
            amy_process_bus_post_hook(bus);
#ifdef AMY_ESP_LOAD_DIAGNOSTIC
            fill_bus_post_us +=
                (uint32_t)(amy_get_us() - hook_started_us);
#endif
        }  // end of serial per-bus FX/hooks
    }

#ifdef AMY_ESP_LOAD_DIAGNOSTIC
    for (uint8_t slot = 0; slot < AMY_MAX_CORES; ++slot) {
        fill_bus_fx_us += fill_bus_fx_current_us[slot];
        fill_bus_send_us += fill_bus_send_current_us[slot];
    }
#endif
